...
function statusError(status: number, context: 'init' | 'authenticate'): CosmoLoginError {
  ...
  if (status === 429) return new CosmoLoginError(t('errorTooManyAttempts', locale));
  if (context === 'authenticate' && (status === 400 || status === 401 || status === 422)) {
    return new CosmoLoginError(t('errorCodeInvalid', locale));
  }
  if (context === 'init' && (status === 400 || status === 422)) {
    return new CosmoLoginError(t('errorEmailInvalid', locale));
  }
  return new CosmoLoginError(t('errorCouldNotReach', locale));
}
...
async function proxyPost(path: string, body: unknown): Promise<Response> {
  try {
    ...
    throw new CosmoLoginError(t('errorCouldNotReach', getLocale()));
  }
}
...
export async function refreshCosmoSession(refreshToken: string): Promise<CosmoCredentials> {
  ...
  if (response.status === 400 || response.status === 401) {
    throw new CosmoSessionExpiredError(t('errorSessionExpired', getLocale()));
  }
  if (!response.ok) throw new CosmoLoginError(t('errorCouldNotReach', getLocale()));
