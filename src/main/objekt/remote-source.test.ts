...
// protocol.registerSchemesAsPrivileged, and (via cosmo-auth.ts -> ../locale)
// ipcMain.handle(), at those modules' own load time — none of the functions
// under test here exercise any of it, but all three still have to resolve
// outside the real Electron runtime.
vi.mock('electron', () => ({
  net: { fetch: vi.fn() },
