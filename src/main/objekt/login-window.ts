...
function loginErrorMessage(error: unknown): string {
  return error instanceof CosmoLoginError ? error.message : t('errorSomethingWrong', getLocale());
}
...
function nicknameErrorMessage(): string {
  return t('loginNicknameNotFound', getLocale());
}
...
function appleScriptTextPrompt(message: string, title: string, confirmLabel: string): Promise<string | undefined> {
  return new Promise((resolve) => {
    const cancelLabel = escapeAppleScriptString(t('loginCancel', getLocale()));
    const script =
      `display dialog "${message}" with title "${title}" default answer "" ` +
      `buttons {"${cancelLabel}", "${confirmLabel}"} default button "${confirmLabel}" cancel button "${cancelLabel}"`;
...
function appleScriptRetryAlert(message: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    const cancelLabel = escapeAppleScriptString(t('loginCancel', locale));
    const tryAgainLabel = escapeAppleScriptString(t('loginTryAgain', locale));
    const script =
      `display alert "${escapeAppleScriptString(message)}" ` +
      `buttons {"${cancelLabel}", "${tryAgainLabel}"} default button "${tryAgainLabel}" cancel button "${cancelLabel}"`;
    execFile('osascript', ['-e', script], (error, stdout) => {
      if (error) {
        resolve(false);
        return;
      }
      resolve(stdout.includes(tryAgainLabel));
    });
  });
}
...
async function loginViaAppleScript(): Promise<void> {
  const locale = getLocale();
  const email = (
    await appleScriptTextPrompt(t('loginEmailPrompt', locale), t('loginWindowTitle', locale), t('loginContinue', locale))
  )?.trim();
  if (!email) return;
  ...
  for (;;) {
    const code = (
      await appleScriptTextPrompt(t('loginCodePrompt', locale), t('loginWindowTitle', locale), t('loginSignIn', locale))
    )?.trim();
    if (!code) return;
  ...
  for (;;) {
    const nickname = (
      await appleScriptTextPrompt(t('loginNicknamePrompt', locale), t('loginWindowTitle', locale), t('loginFinish', locale))
    )?.trim();
    if (!nickname) return;
...
ipcMain.handle(IpcChannel.LoginSendCode, async (_event, email: unknown): Promise<LoginResult> => {
  if (typeof email !== 'string' || !email.trim()) {
    return { ok: false, error: t('loginEnterEmail', getLocale()) };
  }
  try {
...
ipcMain.handle(IpcChannel.LoginVerifyCode, async (_event, payload: unknown): Promise<LoginResult> => {
  ...
  if (typeof email !== 'string' || !email.trim() || typeof code !== 'string' || !code.trim()) {
    return { ok: false, error: t('loginEnterCode', getLocale()) };
  }
  try {
...
ipcMain.handle(IpcChannel.LoginResolveNickname, async (_event, payload: unknown): Promise<LoginResult> => {
  ...
  if (typeof email !== 'string' || !email.trim() || typeof nickname !== 'string' || !nickname.trim()) {
    return { ok: false, error: t('loginEnterNickname', getLocale()) };
  }
  try {
...
function loginViaPromptWindow(parent: BrowserWindow): Promise<void> {
  ...
  return new Promise((resolve) => {
    ...
    promptWindow = new BrowserWindow({
      ...
      modal: true,
      show: false,
      title: t('loginWindowTitle', getLocale()),
      webPreferences: {
