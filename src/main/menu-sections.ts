...
import { lastCachedAt, shuffleEveryMinutes, t } from '@shared/i18n';
...
import { SIZE_PRESETS } from './window';
...
const DIRECTIONS: Array<Settings['spin']['direction']> = ['cw', 'ccw'];
function directionLabel(direction: Settings['spin']['direction']): string {
  return t(direction === 'cw' ? 'directionCw' : 'directionCcw', getLocale());
}
...
export function shuffleLabel(minutes: number | null): string {
  ...
  return minutes === null ? t('shuffleNever', locale) : shuffleEveryMinutes(minutes, locale);
}

const SIZE_PRESET_KEYS = { xs: 'sizeXs', s: 'sizeS', m: 'sizeM', l: 'sizeL', xl: 'sizeXl' } as const;
function sizeLabel(preset: Settings['window']['sizePreset']): string {
  return t(SIZE_PRESET_KEYS[preset], getLocale());
}

export function buildNextObjektItem(): MenuItemConstructorOptions {
  return {
    label: t('menuNextObjekt', getLocale()),
...
export function buildPauseSpinningItem(settings: Settings): MenuItemConstructorOptions {
  return {
    label: t('menuPauseSpinning', getLocale()),
...
export function buildSpinSpeedItem(settings: Settings): MenuItemConstructorOptions {
  return {
    label: t('menuSpinSpeed', getLocale()),
...
export function buildSpinDirectionItem(settings: Settings): MenuItemConstructorOptions {
  return {
    label: t('menuSpinDirection', getLocale()),
    icon: menuSymbol('arrow.triangle.2.circlepath'),
    submenu: DIRECTIONS.map(
...
export function buildRefreshCollectionItem(settings: Settings, options: RefreshCollectionOptions): MenuItemConstructorOptions {
  const locale = getLocale();
  return {
    label: options.refreshing ? t('menuRefreshing', locale) : t('menuRefreshCollection', locale),
...
function lastCachedLabel(cachedAt: number | null): string {
  const locale = getLocale();
  return cachedAt === null ? t('menuLastCachedNever', locale) : lastCachedAt(new Date(cachedAt).toLocaleString(), locale);
}
...
export function buildChooseObjektsItem(win: BrowserWindow): MenuItemConstructorOptions {
  return {
    label: t('menuChooseObjekts', getLocale()),
