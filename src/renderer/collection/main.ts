...
function populateMemberSelect(): void {
  ...
  memberSelect.innerHTML = '<option value="">All</option>';
...
function render(): void {
  ...
  if (allObjekts.length === 0) {
    ...
    empty.textContent = 'No objekts loaded yet. Connect a Cosmo account first.';
    gridEl.append(empty);
  } else if (shown.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'empty';
    empty.textContent = 'No objekts match these filters.';
    gridEl.append(empty);
  } else {
  ...
  statusEl.textContent =
    selectedIds.size === 0
      ? `${shown.length} shown of ${allObjekts.length} — none selected means all are in rotation`
      : `${selectedShownCount} of ${shown.length} shown selected · ${selectedIds.size} selected in total`;
}
...
refreshButton.addEventListener('click', () => {
  void (async () => {
    ...
    try {
      ...
      refreshButton.textContent = 'Refresh collection';
    }
  })();
});

void (async () => {
  const [objekts, settings] = await Promise.all([window.pulsar.getObjektList(), window.pulsar.getSettings()]);
  allObjekts = objekts;
